...
#define BACKEDIT @"后退" //后退一步 （这里只做后退一步操作，如果需要可以使用链表，数组记录,稍后有空会添加）
#define NEW @"重新开始" //重新开始
...
- (void)createButtonViewWithFrame:(CGRect)frame{
    ...
    for (int i = 0; i<11; i++) {
        ...
        if (i == 9) {
            myCreateButton.titleLabel.font =  [UIFont systemFontOfSize:10];
            ...
            [myCreateButton setTitleColor:[UIColor redColor] forState:UIControlStateNormal];
